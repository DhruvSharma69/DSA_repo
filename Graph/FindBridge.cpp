...
void GetBridge(graph &g1, int n)
{
    vector<bool> visited(n,false);
    vector<int> parent(n,-1);
    vector<int> minDis(n, INT_MAX); // Initialize with INT_MAX
    vector<int> firstTouch(n, INT_MAX); // Initialize with INT_MAX
    int timer = 0;
    for(int v=0; v<n; v++)
    { 
        if(visited[v] != true)
